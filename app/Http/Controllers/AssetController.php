...
class AssetController extends Controller
{
    public function getasset(Request $request)
    {
        ...
        if (!$item) {
            return redirect('https://assetcache.link/asset?id=' . $request->id);

            /*$response = Http::withUserAgent('Roblox/WinInet')->get('https://assetcache.link/asset?id=' . $request->id);
    ...
    public function robloxredirect(Request $request)
    {
        ...
        if ($request->assetversionid) {
            $response = Http::withUserAgent('Roblox/WinInet')->get('https://assetcache.link/asset?id=' . $request->assetversionid);
            return $response;
        }
    ...
    public function insertasset(Request $request) {
        ...
        if ($request->has('sid')) {
            //http://www.wawah.rocks/Game/Tools/InsertAsset.ashx?nsets=20&type=user&userid=11744447
