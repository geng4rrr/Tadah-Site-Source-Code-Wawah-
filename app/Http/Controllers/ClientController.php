...
class ClientController extends Controller
{
        ...
        if (!$token->user) {
            $script = strtr(
                ...
                [
                    ...
                    '%PLAYERCHARAPP%' => 'http://' . request()->getHttpHost() . '/users/1/character',
                    '%TOKEN%' => $token->token,
                    '%SERVERIP%' => $ip,
        ...
        if ($token->user) {
            $script = strtr(
                ...
                [
                    ...
                    '%CORESCRIPT_PREFRENCE%' => ($token->user->old_cores ? 1871 : 1320),
                    '%SITE%' => request()->getHttpHost(),
                    '%GUEST%' => 'false'
                ]
            );
    ...
    public function placelauncher(Request $request)
    {
        ...
        $test = '{"jobId":"asdadasd","status":2,"joinScriptUrl":"http://tadah.rocks/game/join.ashx?ip=127.0.0.1&port=53640&username=Iago&id=45","authenticationUrl":"http://google.com","authenticationTicket":"atest","message":"null"}';
        return $test;
    }
